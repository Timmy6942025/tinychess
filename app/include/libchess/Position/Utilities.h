...
inline std::optional<Position> Position::from_fen(const std::string& fen) {
    ...
    // The enpassant square must be the square a pawn of the side that just
    // moved (i.e. NOT the side to move) passed over on a double push: the
    // pawn must sit one rank behind the ep square. A malformed ep square
    // (e.g. "c3" with white to move, where no black pawn could have crossed)
    // would otherwise let the move generator fabricate illegal enpassant
    // captures. Reject it by resetting the square.
    if (curr_state.enpassant_square_) {
        const auto stm      = pos.side_to_move();
        const auto ep_rank  = curr_state.enpassant_square_->rank();
        const auto push_rank = (stm == constants::WHITE) ? ep_rank - 1 : ep_rank + 1;
        if (push_rank < 0 || push_rank > 7) {
            curr_state.enpassant_square_.reset();
        } else {
            const auto pawn_sq = Square{push_rank * 8 + curr_state.enpassant_square_->file()};
            const auto pawn    = pos.piece_on(pawn_sq);
            if (!pawn || pawn->type() != constants::PAWN || pawn->color() == stm)
                curr_state.enpassant_square_.reset();
        }
    }
