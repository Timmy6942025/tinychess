...
* Web play from a phone. The board is an open access point called `DOG-CHESS` at `http://192.168.4.1`. You tap to move, clocks run, and the page works offline. It handles promotion, captured material, last-move and check highlights, move animation, sounds and haptics, flip board, resign, and Fischer clocks. Pick a time control: `1+0` and `2+1` bullet, `3+0` `3+2` `5+0` `5+3` blitz, `10+0` `15+10` rapid, `30+0` classical. Increment is added after each move, like lichess and chess.com. One game at a time. Others see the position live and can join a waitlist. The holder can rematch or give up the board. Idle seats free after 3 minutes.
...
* Clocks are server authoritative. The server deducts your think on each move plus its own, adds the Fischer increment for the chosen time control, and flags at zero. The page just renders the countdown.
