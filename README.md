...
* Search that survived measurement. The stacked gated total is about +242 Elo vs Tier-1: recalibrated LMR (+34.6), TT aging (+36.7), mate distance pruning (+19.8), razoring (+12.2), check extension (about +75), qsearch SEE pruning (+29.6), recapture extension (+57.8), qsearch TT probe removal (+34.9), a move-ordering rebuild (+24.6 accepted, then +16.5 +/- 16.3 on an 800-game replication) that added capture history, butterfly from-to history, and a one-ply continuation table next to the old piece-to history, a square-major NNUE weight layout (+6.9, LOS 67.1%, clustering feature rows by square so the PSRAM stream stays local), and a 128 KB SRAM L0 transposition table in front of the PSRAM TT (+5.2, LOS 62.6%). Design notes in `docs/move-ordering-rebuild.md`. The rejects are kept with numbers too: killers, hist3d, late-move pruning, correction history, the later persistent shared correction-history plus experience-table attempt, delta pruning, eval cache, a shelf of margin probes, a phase-2 sweep that gated a staged move picker, easy-move/stability time management, proper singular extensions, and a 32-bit-signature transposition table against the current state and reverted all four, a ProbCut verification pruner on Aug 29 that lost 520 to 655 Elo at margins 100/150/200 over 200 games each (the search sits at a measured local optimum), and the September speed-campaign gates: `-DNDEBUG` (-19.1), per-node sort-stack scores (-6.9), insertion sort (-33.1, ordering got worse), split-brain per-core TT (-29.6, shared knowledge beat coherence savings), and TT-score-before-static-eval (-34.9, searched bounds distort the pruning margins). Everything lives in `tools/results.log`.
* Speed that we actually measured. Native with LTO and the big net does roughly 350k to 590k nps on the dev box by the bench protocol depending on machine load (instantaneous UCI rates run higher). The ordering-table reads cost about 6% of that; the strength they buy is measured at more than that in Elo. An Aug 26 re-profile (tools/results.log) re-confirmed the tax at 5.7% desktop and 3.3% board (alternating-flash 18,916 -> 18,283) and tried to reclaim it bit-exact: victim reuse and bounds cleanup saved 0.40% of instructions and ~0% on the board, and the naive 2 MB merged butterfly/continuation layout does not fit the 33 KB PSRAM budget, so no layout ship. The board does about 17.5k nps cool and about 15.0k warm on the startpos bench (64-byte cache lines, Sep 18) (compare same-session pairs only, silicon drifts ~15% with temperature), up from ~8,300 before the evaluator kernel rebuild (worth a measured +174 +/- 34 Elo on-device) and ~17,700 before the output layer moved into on-board SRAM (+6% node throughput, bit-exact, see docs/output-layer-staging.md). Earlier history: the gain from 5,079 to 6,857 came mostly from a clean rebuild and two small fixes. C2 moved the WDT gate to 1.5 s and kept. C9 reordered weights square-major and measured +24% board bench cool (14,213 -> 17,609). C11 added the SRAM L0 TT at parity. Rejected: C5 sort-stack scores (-6.9 Elo), C4 QIO flash (hung under 2-thread load, and the speed claim was a measurement error). Flash stays DIO.
* Web play from a phone. The board is an open access point called `DOG-CHESS` at `http://192.168.4.1`. You tap to move, clocks run, and the page works offline. While the engine thinks you can queue a premove that plays itself the moment its reply lands (cancelled loudly if the reply makes it illegal). It handles promotion, captured material, last-move and check highlights, move animation, sounds and haptics, flip board, resign, pause and resume with frozen clocks, and Fischer clocks. Pick a time control: `1+0` and `2+1` bullet, `3+0` `3+2` `5+0` `5+3` blitz, `10+0` `15+10` rapid, `30+0` classical. Increment is added after each move, like lichess and chess.com. One game at a time. Others see the position live with a presence count and can join a waitlist. The holder can rematch or give up the board. Idle seats free after 3 minutes. The engine thinking indicator is a masked sweep animation, moves publish to spectators in about one poll, and a PGN copy button, sound toggle, and offline banner round out the page.
...
* Vendored `app/include/libchess` with local fixes for `pseudo_legal_move_list_into`, FEN en-passant validation, `go st` UCI support, tolerant `go`-line numbers (unknown tokens skipped, present-but-zero clocks floor at 1 ms instead of searching unbounded), and refusal of piece-count-illegal FENs (a corrupt opening line with 17 pieces overflowed the eval staging buffer and aborted; the handler keeps the previous position now). The canonical copy is at `github.com/Timmy6942025/libchess`.
...
Get the latest published release, `v0.8-prebuilt`, built from commit `bd1d96e`, from https://github.com/Timmy6942025/tinychess/releases.
...
4. Pick white or black, pick a time control, tap Start game.
5. Tap a piece, then tap a target. Drag also works. The piece you hold follows your finger. Promotions pop a chooser.
6. Use Flip board to view from either side, Pause to freeze both clocks mid-game, Resign to end early. After mate, stalemate, draw, or flag fall the result card offers Rematch, Give up the board, or View board.
...
The page is one file, `app/data/web/index.html`, about 79 KB including the PNG pieces under `pieces/ejgfv`. No CDN, no build step.

* Board is a CSS grid, 8 by 8. In portrait it fills the full width and stays square, the page scrolls if the screen is short so the board stays large. In landscape on short phones the board is sized from the viewport height and sits left of the clocks and controls.
* State comes from the board. `GET /state` returns FEN, legal moves, clocks, move list, game-over flag, plus `paused`, `searching`, and client count for every device. `POST /move` runs the search (async: the human move publishes instantly, the engine reply lands on the next poll). `POST /pause` and `POST /resume` freeze and release both clocks, owner only. `GET /battery` returns millivolts. Your browser stores a random pid in localStorage so the board can tell who holds the seat.
* Clocks are server authoritative. The server deducts your think on each move plus its own, adds the Fischer increment for the chosen time control, and flags at zero. Pause shifts the turn anchor so frozen time is never charged. The page just renders the countdown.
* Assets are served from SPIFFS. `web.cpp` sends `Cache-Control: no-cache` so a reflash is visible after one hard refresh.

Pieces are the ejgfv set. The board colors are `f0d9b5` and `b58863`, with a dark theme around them.
