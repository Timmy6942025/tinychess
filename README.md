...
* Speed that we actually measured. Native with LTO and the big net does about 340k nps on the dev box by the bench protocol (instantaneous UCI rates run higher), 150 to 290k under load. The board does about 14,700 nps average on the startpos bench and near 17k instantaneous, up from ~8,300 before the evaluator kernel rebuild - worth a measured +174 +/- 34 Elo on-device. Earlier history: the gain from 5,079 to 6,857 came mostly from a clean rebuild and two small fixes. C2 moved the WDT gate to 1.5 s and C5 fixed per-node allocations. C4 tried QIO flash and reverted. It hung under 2-thread load and the speed claim was a measurement error.
