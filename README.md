...
* Vendored `app/include/libchess` with local fixes for `pseudo_legal_move_list_into`, FEN en-passant validation, `go st` UCI support, and tolerant `go`-line numbers (unknown tokens skipped, present-but-zero clocks floor at 1 ms instead of searching unbounded). The canonical copy is at `github.com/Timmy6942025/libchess`.
