...
The board is where compute actually binds, and there the same change is worth a lot: bench went from ~8,200 to ~14,700 average nps (~17k instantaneous), fixed-depth searches now match the desktop binary digit-for-digit through depth 8, and a 240-game self-play match giving one side exactly the achieved 1.8x clock ratio measured the gain at **+173.9 +/- 34.3, LOS 100%**. The evaluator's cost curve kept paying after that: profiling showed the output layer's 1 KB of weights were re-read through flash XIP on every single eval, so they now stage into 1088 bytes of internal SRAM at boot - bit-exact by construction, graceful fallback if the allocation loses, worth another **+6% node throughput** in a same-day A/B (17.7k -> 18.8k). Design notes in `docs/output-layer-staging.md`.
...
- The latest published prebuilt release (`v0.3-prebuilt`, built from `4749c02`) ships the desktop binary and a flashable board image so a new owner needs Python and a cable, nothing else. `main` has newer source changes and has not been packaged into a release yet.
